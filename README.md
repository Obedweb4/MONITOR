<h1 align="center">🟢 OBEDTECH MONITOR</h1>
<p align="center"><b>Keep free-hosted sites awake. Know the moment yours goes down — before your users do.</b></p>

<p align="center">
  <a href="https://monitor.obedtech.top"><img src="https://img.shields.io/badge/LIVE%20APP-monitor.obedtech.top-green?style=for-the-badge&logo=googlechrome" alt="Live App"/></a>
</p>
...
**OBEDTECH MONITOR** solves two real problems developers face with free hosting:

1. **Free-tier sleep** — Platforms like Render, Railway, and Fly.io put your app to sleep after a period of inactivity. OBEDTECH MONITOR pings your service at your chosen interval so it stays awake and ready to respond instantly.

2. **Silent downtime** — When your deployed site goes down, you're usually the last to know — after your users do. OBEDTECH MONITOR detects the outage immediately and sends you an email alert so you can fix it before anyone else notices.
...
**Live at:** [https://monitor.obedtech.top](https://monitor.obedtech.top)
...
**1. Clone the repo**
```bash
git clone https://github.com/mauricegift/obedtech-monitor.git
cd obedtech-monitor
```
...
```env
DATABASE_URL=mongodb+srv://user:password@cluster.mongodb.net/obedtech-monitor
```
...
CREATE DATABASE obedtech_monitor OWNER monitor_user;
GRANT ALL PRIVILEGES ON DATABASE obedtech_monitor TO monitor_user;
\q
```

**3. Set DATABASE_URL in your `.env`**
```env
DATABASE_URL=postgresql://monitor_user:strongpassword@localhost:5432/obedtech_monitor
```
...
The `From` address on all emails will be `OBEDTECH MONITOR <no-reply@alerts.yourdomain.com>`.
...
```bash
curl -X POST https://monitor.obedtech.top/api/apikeys \
...
**Listing keys:**
```bash
curl https://monitor.obedtech.top/api/apikeys \
  -H "Authorization: Bearer YOUR_JWT"
```

**Revoking a key:**
```bash
curl -X DELETE https://monitor.obedtech.top/api/apikeys/KEY_ID \
  -H "Authorization: Bearer YOUR_JWT"
```
...
**Base URL:** `https://monitor.obedtech.top/api/v1`
...
**Node.js**
```javascript
const BASE = 'https://monitor.obedtech.top/api/v1';
...
BASE = 'https://monitor.obedtech.top/api/v1'
...
```bash
# List monitors
curl https://monitor.obedtech.top/api/v1/monitors \
  -H "X-API-Key: gm_your_api_key_here"

# Create a monitor
curl -X POST https://monitor.obedtech.top/api/v1/monitors \
  -H "X-API-Key: gm_your_api_key_here" \
  -H "Content-Type: application/json" \
  -d '{"name":"My API","url":"https://api.example.com","interval_mins":5}'

# Delete a monitor
curl -X DELETE https://monitor.obedtech.top/api/v1/monitors/42 \
  -H "X-API-Key: gm_your_api_key_here"
```
...
- Select your `obedtech-monitor` repo
...
| `FRONTEND_URL` | Your Railway public URL, e.g. `https://obedtech-monitor.up.railway.app` |
...
```markdown
[![Deploy](https://www.herokucdn.com/deploy/button.svg)](https://heroku.com/deploy?template=https://github.com/mauricegift/obedtech-monitor)
```
...
4. Fill in the prompted `sync: false` variables:
   - `FRONTEND_URL` → your Render URL, e.g. `https://obedtech-monitor.onrender.com`
   - `RESEND1_API_KEY` → your Resend API key
...
2. Set:
   ...
   - **Environment:** Node
3. **New** → **PostgreSQL** → create a database named `obedtech-monitor-db`
...
```bash
git clone https://github.com/mauricegift/obedtech-monitor.git /srv/obedtech-monitor
cd /srv/obedtech-monitor
...
```bash
npm install -g pm2
pm2 start backend/index.js --name obedtech-monitor
pm2 save
...
sudo nano /etc/nginx/sites-available/obedtech-monitor
```
...
```bash
sudo ln -s /etc/nginx/sites-available/obedtech-monitor /etc/nginx/sites-enabled/
...
<p align="center">Built by <a href="https://me.obedtech.top">Maurice Gift</a> · <a href="https://obedtech.top">OBED TECH</a></p>
